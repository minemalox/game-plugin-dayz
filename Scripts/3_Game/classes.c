...
class _Event {
    ...
    void _Event(string className, string icon, Object _reference) {this.className = className; this.icon = icon; _reference.GetNetworkID(id_low, id_high); this.id = id_high.ToString() + id_low.ToString(); }
    string GetID() { return this.id; }
    string Class() { return this.className; }
    string Icon() { return this.icon; }
    Object Ref() { return GetGame().GetObjectByNetworkId(id_low, id_high); }
